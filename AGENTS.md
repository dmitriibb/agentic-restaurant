...
12. Shared React UI primitives live in `apps/ui-common-libs`. When changing `orders-client`, `staff-client`, or future web UIs, prefer imports from that package and extend it first instead of introducing app-local component variants or direct raw `@mui/material` usage.
