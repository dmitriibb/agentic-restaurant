...
// Register mounts the production API routes on the given mux behind the auth middleware.
func (h *Handlers) Register(mux *http.ServeMux, authMiddleware func(http.Handler) http.Handler) {
	protected := http.NewServeMux()
	protected.HandleFunc("GET /api/v1/production/orders", h.listOrders)
	protected.HandleFunc("GET /api/v1/production/orders/{orderId}", h.getOrder)
	protected.HandleFunc("POST /api/v1/production/items/{itemId}/pickup", h.commandHandler(domain.CommandPickup))
	protected.HandleFunc("POST /api/v1/production/items/{itemId}/block", h.commandHandler(domain.CommandBlock))
	protected.HandleFunc("POST /api/v1/production/items/{itemId}/resume", h.commandHandler(domain.CommandResume))
	protected.HandleFunc("POST /api/v1/production/items/{itemId}/ready", h.commandHandler(domain.CommandReady))

	mux.Handle("/api/v1/production/", authMiddleware(protected))
}
